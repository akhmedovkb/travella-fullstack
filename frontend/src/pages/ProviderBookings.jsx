...
export default function ProviderBookings() {
  ...
  const confirmOutgoing = async (b) => {
    try {
      await axios.post(
        `${API_BASE}/api/bookings/${b.id}/confirm-by-requester`,
        {},
        cfg()
      );
  ...
  const cancelOutgoing = async (b) => {
    try {
      await axios.post(
        `${API_BASE}/api/bookings/${b.id}/cancel-by-requester`,
        {},
        cfg()
      );
    ...
    return (
      <div className="space-y-4">
        {list.map((b) => {
          ...
          const awaitingRequester =
            isIncoming && alreadyQuoted && String(b?.status) === "pending";
          return (
              ...
              {awaitingRequester && (
                <div className="mt-2 rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-800">
                  {t("bookings.waiting_for_requester", {
                    defaultValue:
                      "Предложение отправлено. Ожидаем подтверждения клиента/заявителя.",
                  })}
                </div>
              )}
            </div>
    ...
    <div className="mx-auto max-w-5xl p-4 md:p-6">
      ...
      <div className="mb-4 flex flex-wrap gap-2">
        ...
        >
          ...
          <span
            className={
              "ml-2 inline-flex items-center rounded-full px-1.5 text-xs " +
              (tab === "incoming" ? "bg-white/20" : "bg-gray-100")
            }
          >
            {incomingCount}
          </span>
        ...
        >
          ...
          <span
            className={
              "ml-2 inline-flex items-center rounded-full px-1.5 text-xs " +
              (tab === "outgoing" ? "bg-white/20" : "bg-gray-100")
            }
          >
            {outgoingCount}
          </span>
