      ...
      <div className="space-y-4">
        {list.map((b) => {
          ...
          if (String(b.status) === "rejected") {
            ...
            if (isIncoming) {
              // входящие: отменил клиент/заявитель (клиент или провайдер-заявитель)
              cancelledByLabel = t("bookings.cancelled_by_client", { defaultValue: "клиентом/заявителем" });
            } else {
              // исходящие: отменил я как заявитель
              cancelledByLabel = t("bookings.cancelled_by_you", { defaultValue: "вами (заявителем)" });
            }
          }
          ...
          return (
            <div key={b.id} className="rounded-xl border bg-white p-3">
              <BookingRow
                ...
                rejectedByLabel={rejectedByLabel}        // КЕМ ОТКЛОНЕНО (для rejected)
                cancelledByLabel={cancelledByLabel}      // КЕМ ОТМЕНЕНО (для cancelled)
                onAccept={accept}
                onReject={reject}
                onCancel={cancelOutgoing}
              />

              {/* Входящие: моя форма согласования цены (прячем после оффера) */}
