...
function extractServiceFields(item) {
  // элемент избранного может прийти как чистая услуга или { service, service_id, ... }
...
function resolveExpireAt(s) {
  const d = s?.details || {};
  const cand = [s?.expires_at, s?.expire_at, s?.expireAt, d.expires_at, d.expire_at, d.expiresAt, d.expiration, d.expiration_at, d.expirationAt, d.expiration_ts, d.expirationTs]
    .find((v) => v !== undefined && v !== null && String(v).trim?.() !== "");
  let ts = null;
  if (cand !== undefined && cand !== null) {
    if (typeof cand === "number") ts = cand > 1e12 ? cand : cand * 1000;
    else { const p = Date.parse(String(cand)); if (!Number.isNaN(p)) ts = p; }
  }
  if (!ts) {
...
function renderTelegram(value) {
  ...
  const s = String(value).trim(); let href = null; let label = s;
...
export default function ProviderFavorites() {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        setItems(arr);
        const ids = arr.map((x) => x.service_id ?? x.service?.id ?? x.id).filter(Boolean).map(String);
  ...
  const toggleFavorite = async (serviceId) => {
    ...
    // оптимистично (мгновенно) переворачиваем
    ...
    try {
      ...
      // если сервер сказал обратное — подправим
      ...
      toast(res?.added ? (t("favorites.added_toast") || "Добавлено в избранное")
      ...
      if (!flipTo) {
        // если откатываем снятие — вернём карточку
        const lost = items.find(x => String(x.service_id ?? x.service?.id ?? x.id) === key);
        if (!lost) setItems(prev => prev); // no-op
      }
  ...
  const Card = ({ it, now }) => {
    const { svc, title, hotel, accommodation, dates, prettyPrice, inlineProvider, providerId, status: statusRaw, details } = extractServiceFields(it);
    ...
    // provider/contact display (минимум как в Marketplace)
    const tg = renderTelegram(inlineProvider?.telegram || inlineProvider?.tg || inlineProvider?.telegram_username || inlineProvider?.telegram_link);
        ...
        <div className="aspect-[16/10] bg-gray-100 relative">
          ...
          <div className="absolute top-2 left-2 right-2 flex items-center justify-between pointer-events-none">
            <div className="flex items-center gap-2">
              {hasTimer && (
                <span className={`pointer-events-auto px-2 py-0.5 rounded-full text-white text-xs backdrop-blur-md ring-1 ring-white/20 shadow ${leftMs > 0 ? "bg-orange-600/95" : "bg-gray-400/90"}`} title={leftMs > 0 ? "До окончания" : "Время истекло"}>
                  {timerText}
                </span>
              )}
              <button
            ...
            <div className="pointer-events-auto">
              <button type="button"
                ...
                <svg width="18" height="18" viewBox="0 0 24 24"
                     className="relative"
                     fill={isFav ? "#ef4444" : "none"} stroke={isFav ? "#ef4444" : "currentColor"} strokeWidth="2">
          ...
          {/* hover-подложка с краткими данными */}
        ...
        <div className="p-3 flex-1 flex flex-col">
          ...
          {tg?.label && (
            <div className="mt-2 text-sm">
              ...
              {tg.href ? <a href={tg.href} target="_blank" rel="noopener noreferrer" className="underline">{tg.label}</a>
