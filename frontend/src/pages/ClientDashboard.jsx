...
function MyInsideCard({ inside, loading, t, onJoined, now }) {
  ...
  const programDaysMap = {
    royal: [
      t("landing.inside.program.royal.d1", { defaultValue: "Дели: прилёт, трансфер, вечерний брифинг" }),
      t("landing.inside.program.royal.d2", { defaultValue: "Агра: Тадж-Махал на рассвете, форт Агры" }),
      t("landing.inside.program.royal.d3", { defaultValue: "Джайпур: Амбер-форт, Дворец ветров" }),
      t("landing.inside.program.royal.d4", { defaultValue: "Джайпур: тур по городу, ремёсла" }),
      t("landing.inside.program.royal.d5", { defaultValue: "Дели: современная Индия — арт/мода/гастро" }),
      t("landing.inside.program.royal.d6", { defaultValue: "Свободный день / доп. опции" }),
      t("landing.inside.program.royal.d7", { defaultValue: "Вылет" }),
    ],
    silence: [
      t("landing.inside.program.silence.d1", { defaultValue: "Удайпур: прилёт, озеро Пичола, City Palace" }),
      t("landing.inside.program.silence.d2", { defaultValue: "Удайпур: храмы Джагдиш/Эклингджи, ремёсла, вечерний круиз" }),
      t("landing.inside.program.silence.d3", { defaultValue: "Переезд в Джодхпур, прогулка по «синему» старому городу" }),
      t("landing.inside.program.silence.d4", { defaultValue: "Джодхпур: форт Мехрангарх, башни, закат на крыше" }),
      t("landing.inside.program.silence.d5", { defaultValue: "Переезд в Джайсалмер, Золотой форт, хавели" }),
      t("landing.inside.program.silence.d6", { defaultValue: "Пустыня Тар: сафари на дюнах, ужин у костра" }),
      t("landing.inside.program.silence.d7", { defaultValue: "Резерв/отдых или мастер-класс" }),
      t("landing.inside.program.silence.d8", { defaultValue: "Вылет" }),
    ],
    modern: [
      t("landing.inside.program.modern.d1", { defaultValue: "Мумбаи: полудневной тур, вечер на набережной" }),
      t("landing.inside.program.modern.d2", { defaultValue: "Студии Болливуда/арт-кварталы" }),
      t("landing.inside.program.modern.d3", { defaultValue: "Перелёт в Гоа, океан" }),
      t("landing.inside.program.modern.d4", { defaultValue: "Пляжи, старый Гоа, португальское наследие" }),
      t("landing.inside.program.modern.d5", { defaultValue: "Яхта/закат, гастрономический вечер" }),
      t("landing.inside.program.modern.d6", { defaultValue: "Свободный день / wellness" }),
      t("landing.inside.program.modern.d7", { defaultValue: "Вылет" }),
    ],
    kerala: [
      t("landing.inside.program.kerala.d1", { defaultValue: "Кочи: город, китайские сети, колониальная часть" }),
      t("landing.inside.program.kerala.d2", { defaultValue: "Муннар: чайные плантации, viewpoints" }),
      t("landing.inside.program.kerala.d3", { defaultValue: "Муннар: треккинг / фотодень" }),
      t("landing.inside.program.kerala.d4", { defaultValue: "Аллеппи: заселение на хаусбоат" }),
      t("landing.inside.program.kerala.d5", { defaultValue: "Бэквотеры, деревни, локальная кухня" }),
      t("landing.inside.program.kerala.d6", { defaultValue: "Аюрведический центр, индивидуальная программа" }),
      t("landing.inside.program.kerala.d7", { defaultValue: "Свободный день / море" }),
      t("landing.inside.program.kerala.d8", { defaultValue: "Вылет" }),
    ],
  };
  ...
  // все главы для дат + прогресса по дням
  useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
        ...
        if (!cancelled) setChaptersPublic(items);
      } catch {
        if (!cancelled) setChaptersPublic(null);
      } finally {
  ...
  const chaptersPublicByKey = useMemo(() => {
    const map = {};
    (chaptersPublic || []).forEach((c) => {
      if (c && c.chapter_key) {
        map[String(c.chapter_key)] = c;
      }
    });
    return map;
  }, [chaptersPublic]);
  ...
  const calcTourProgressPct = (chapterKey) => {
    const meta =
      chaptersPublicByKey[chapterKey] ||
      (inside?.chapter && (inside.chapter.key === chapterKey || inside.chapter.chapter_key === chapterKey)
        ? inside.chapter
        : null);

    const startRaw = meta?.tour_starts_at;
    const endRaw = meta?.tour_ends_at;
    ...
    // после окончания тура считаем главу полностью пройденной
    if (nowTs >= end) return 100;

    const total = end - start;
    const done = nowTs - start;
  ...
  const selectedChapterMeta = chaptersPublicByKey[selectedKey] || null;

  const startsAtRaw = selectedChapterMeta?.starts_at || null;
  ...
  const capacity = selectedChapterMeta?.capacity != null ? Number(selectedChapterMeta.capacity) : null;
  const enrolled = selectedChapterMeta?.enrolled_count != null ? Number(selectedChapterMeta.enrolled_count) : 0;
  const remaining =
    ...
    <section className="bg-white rounded-xl shadow p-6 border">
      ...
      <div className="mt-6 rounded-2xl bg-slate-50 border border-slate-200 px-4 py-3">
        ...
        <div className="mt-3 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {chaptersOrder.map(({ key, order }) => {
            const pct = calcTourProgressPct(key);
            return (
              <div key={key} className="flex flex-col gap-1">
                {/* батарейка */}
                <div className="rounded-full border border-rose-200 bg-white/80 px-1 py-1 flex items-center gap-[2px]">
                  <div className="flex-1 h-3 rounded-full bg-rose-100 overflow-hidden">
                    <div
                      className="h-3 bg-rose-500 transition-all"
                      style={{ width: `${pct}%` }}
                    />
                  </div>
                  {/* «носик» батарейки */}
        ...
        <div className="mt-6" />
          ...
          <div className="mt-6 bg-white rounded-xl shadow p-6 border">
            <div className="flex items-center gap-3 border-b pb-3 mb-4">
              ...
              >
                {t("client.tabs.balance", { defaultValue: "Баланс" })}
              </Link>
