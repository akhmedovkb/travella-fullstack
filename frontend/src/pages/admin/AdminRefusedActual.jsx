...
export default function AdminRefusedActual() {
  ...
  const selectedOutsideVisibleCount = Math.max(0, selectedIds.length - selectedVisibleCount);
          ...
          <>
            <div className="mt-4 flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-3 shadow-sm md:flex-row md:items-center md:justify-between">
              <label className="inline-flex items-center gap-2 text-sm font-bold text-slate-800">
                ...
                />
                Выбрать страницу ({selectedVisibleCount}/{selectableVisibleIds.length})
              </label>
              <button
                type="button"
              ...
              <div className="flex flex-wrap items-center gap-2">
                <span
                  ...
                  title={
                    selectedOutsideVisibleCount
                      ? `Ещё выбрано вне текущей страницы: ${selectedOutsideVisibleCount}`
                      : "Все выбранные находятся на текущей странице"
                  }
                >
                  Выбрано: {selectedVisibleCount} на странице / {selectedIds.length} всего
                </span>
                <button
                  type="button"
                ...
                <button
                  ...
                  disabled={!selectedVisibleCount || bulkSending}
                  className="rounded-xl border border-slate-200 px-3 py-2 text-xs font-bold text-slate-700 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50"
                >
                  Снять на странице
                </button>
