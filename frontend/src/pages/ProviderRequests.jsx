...
export default function ProviderRequests() {
  ...
  async function load() {
    setLoading(true);
    try {
      // для провайдера вернёт заявки по его услугам
      const rows = await apiGet("/api/requests/my", "provider");
      setItems(rows || []);
    } catch (e) {
      console.error(e);
      setItems([]); // страховка
            ...
            {list.map((r) => (
              <div key={r.id} className="border rounded-lg p-4">
                ...
                <div className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-3">
                  <div className="flex gap-2">
                    <input
                      ...
                      onChange={(e) =>
                        setReplyTextById((p) => ({ ...p, [r.id]: e.target.value }))
                      }
                    />
                    <button
                  ...
                  <div>
                    <textarea
                      ...
                      onChange={(e) =>
                        setProposalById((p) => ({ ...p, [r.id]: e.target.value }))
                      }
                    />
                ...
                <div className="mt-2 text-xs text-gray-500">
                  После того как клиент примет предложение (status = accepted), он
                  сможет оформить бронирование.
                </div>
              </div>
            ))}
          </div>
