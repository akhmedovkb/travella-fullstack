...
const renderCreateForm = () => {
  if (["refused_tour", "author_tour"].includes(category) && profile.type === "agent") {
    return (
      <>
        ...
        {/* Категория номера, размещение, ADT/CHD/INF */}
        <div className="mb-4">
  ...
  // 👉 Универсальная форма для гида / транспорта / отеля
  return (
    <>
      <input
        value={title}
...
return (
  <>
    ...
    <div>{/* сообщение после сохранения */}
    {messageService && (
      <p className="text-sm text-center text-gray-600 mt-4">{messageService}</p>
    )}
   </div>
    {/* календарь занятых дат (только для guide/transport) */}
