...
export default function ProviderProfile() {
  ...
  const { id } = useParams();             // provider id из маршрута
  const [sp] = useSearchParams();         // например, service_id/request_id
  const serviceId = sp.get("service_id") ? Number(sp.get("service_id")) : undefined;
  const requestId = sp.get("request_id") || undefined;

  const [role] = useState(getRole());
  const canReview = role === "client";    // отзыв провайдеру могут оставлять клиенты
  ...
  const load = async (p = 1) => {
    setLoading(true);
    try {
      // профиль провайдера — берём с любого живого эндпоинта
      let profile = null;
      const endpoints = [
        `/api/providers/${id}`,
        `/api/provider/${id}`,
      ...
      for (const url of endpoints) {
        try {
          ...
          const obj = (res && (res.data || res.item || res.provider || res.company || res.profile)) || res;
      ...
      const data = await getProviderReviews(id, { limit: pageSize, offset: off });
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            <div className="text-xl font-semibold">
              {prov?.name || prov?.title || t("provider.profile") || "Профиль провайдера"}
            </div>
      ...
      {canReview && (
        <div className="mb-5">
          <ReviewForm
            targetType="provider"
            targetId={Number(id)}
            targetName={prov?.name || prov?.title}
            serviceId={serviceId}
            requestId={requestId}
            onCreated={() => load(1)}
          />
