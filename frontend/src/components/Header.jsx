...
export default function Header() {
  ...
  // --- Logo: через BASE_URL чтобы не пропадало на RU/UZ и поддоменах
  const base = (import.meta?.env?.BASE_URL || "/").replace(/\/?$/, "/");
  const isIndiaPath = location.pathname.startsWith("/india");
  const logoSrc = `${base}logo1.jpg`; // один логотип для всех, позже можно сделать второй для India
  ...
  return (
    <header className="sticky top-0 z-40 bg-[#111] text-white border-b border-black/40">
      {/* паддинги меньше, чтобы лого ушло левее */}
      <div className="mx-auto max-w-7xl px-1 sm:px-2">
        <div className="h-14 flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-2">
            ...
            {/* LOGO — максимально левый, плюс нормальный отступ от навигации */}
            <Link
              to="/"
              className="inline-flex items-center -ml-1 sm:-ml-2 md:-ml-3 mr-3 sm:mr-4"
    ...
    <NavLink
      ...
      className={({ isActive }) =>
        [
          ...
          isActive
            ? "bg-white/10 text-white font-semibold after:content-[''] after:absolute after:left-3 after:right-3 after:-bottom-1 after:h-[2px] after:bg-orange-400 after:rounded-full"
            : "text-white/80 hover:text-white hover:bgwhite/10",
        ].join(" ")
      }
    >
      {icon}
