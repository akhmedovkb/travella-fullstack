...
function buildScript(ctx = {}, options = {}) {
  ...
  const urgencyLines = pickVariant(seed, mode === "aggressive" ? [
    ...
    [
      ...
      "Следующий шаг простой: оплатить доступ к поставщику.",
    ],
  ] : [
    [
  ...
  const ctaLines = pickVariant(seed, mode === "aggressive" ? [
    [
      ...
      "Нажимайте «Оплатить доступ к поставщику» под видео и проверяйте наличие сейчас.",
    ],
    [
      "Подходит по датам и цене?",
      "Жмите «Оплатить доступ к поставщику» под видео и сразу переходите к подтверждению.",
    ],
    [
      "Не откладывайте на вечер.",
      "Кнопка «Оплатить доступ к поставщику» под видео — ваш следующий шаг.",
    ],
  ] : [
    [
      "Хотите этот вариант?",
      "Нажимайте «Оплатить доступ к поставщику» под видео и забирайте тур сейчас.",
    ],
    [
      "Если подходит — не ждите.",
      "Жмите «Оплатить доступ к поставщику» под видео и уточняйте наличие.",
    ],
    [
      "Кнопка под видео уже ждёт.",
      "Нажимайте «Оплатить доступ к поставщику» и забирайте этот вариант.",
    ],
  ], 6);
  const lines = [];
  ...
  if (mode === "short") {
    return [
      ...
      "Нажимайте «Оплатить доступ к поставщику» под видео.",
    ].filter(hasValue).join("\n").replace(/\n{3,}/g, "\n\n").trim();
  }
...
function buildMotionPrompt(ctx = {}, options = {}) {
  ...
  const ctaGesture = pickVariant(seed, [
    "В финале улыбается, уверенно показывает вниз во время фразы про кнопку «Оплатить доступ к поставщику» под видео.",
