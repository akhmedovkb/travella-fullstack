...
async function getClientBalanceSnapshot(client, clientId) {
  ...
  if (balanceCol) {
    const r = await client.query(
      `SELECT COALESCE(${balanceCol}, 0)::bigint AS balance FROM clients WHERE id=$1 LIMIT 1`,
      [clientId]
    );
    balance = Number(r.rows[0]?.balance || 0);
  } else {
    balance = await getBalanceFromLedger(client, clientId);
  }
...
async function createTopupOrder(req, res) {
  ...
  try {
    ...
    const existsQ = await db.query(`SELECT id FROM clients WHERE id=$1 LIMIT 1`, [clientId]);
...
async function unlockContact(req, res) {
  ...
  try {
    ...
    const balanceCol = await getClientsBalanceColumn(db);
    let balance = 0;

    if (balanceCol) {
      const balQ = await db.query(
        `SELECT COALESCE(${balanceCol}, 0)::bigint AS balance FROM clients WHERE id=$1 FOR UPDATE`,
        [clientId]
      );
      balance = Number(balQ.rows[0]?.balance || 0);
    } else {
      balance = await getBalanceFromLedger(db, clientId);
    }
    ...
    if (balanceCol) {
      await db.query(
        `UPDATE clients
            SET ${balanceCol} = COALESCE(${balanceCol}, 0) - $2
          WHERE id = $1`,
        [clientId, price]
      );
    }

    const after = await getClientBalanceSnapshot(db, clientId);
