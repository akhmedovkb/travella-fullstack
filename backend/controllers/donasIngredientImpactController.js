...
exports.getMarginImpact = async (req, res) => {
  ...
  const threshold = req.query.threshold != null ? toNum(req.query.threshold) : 40;
  ...
  const menuItemIds = (mi.rows || []).map((r) => Number(r.menu_item_id)).filter(Boolean);
  if (menuItemIds.length === 0) {
    return res.json({ threshold, items: [], below: [] });
  }

  // тянем рецепты и ингредиенты сразу пачкой
  const recipeRows = await db.query(
    `
    SELECT r.menu_item_id, r.ingredient_id, r.qty, r.unit,
           i.name as ing_name, i.unit as ing_unit, i.pack_size, i.pack_price
  ...
  for (const id of menuItemIds) {
    ...
    for (const line of lines) {
      ...
      const ppu = packSize > 0 ? packPrice / packSize : 0; // цена за 1 unit ингредиента (g/ml/pcs)
  ...
  // сортируем: сначала самые низкие маржи
  out.sort((a, b) => {
  ...
  res.json({ threshold, items: out, below });
};
