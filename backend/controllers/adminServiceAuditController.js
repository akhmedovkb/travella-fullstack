...
async function listServiceAuditLogs(req, res) {
  try {
    ...
    const summarySql = `
      SELECT
        ...
        COUNT(*) FILTER (
          WHERE action IN (
            'service_restored',
            'provider_service_restored'
          )
        )::int restored_count,

        COUNT(*) FILTER (
          WHERE action='service_submitted'
        )::int submitted_count,

        COUNT(*) FILTER (
          WHERE action IN (
            'service_updated',
            'service_status_reset_to_draft'
          )
        )::int updated_count
    ...
    const [rowsRes, countRes, summaryRes, actionRes] =
      await Promise.all([
        pool.query(
          rowsSql,
          [...values, limit, offset]
...
async function getServiceAuditStats(
  req,
  res
) {
  try {
