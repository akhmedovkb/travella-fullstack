...
const priceKind =
  ...
  const shortMealLabel = (value) => {
    ...
    const map = {
      ...
      FBT: "FBT (3-разовое + базовое лечение)",
      AI: "AI (всё включено)",
      UAI: "UAI (ультра всё включено)",
      HALAL: "Halal",
    };
    if (map[up]) return map[up];
    if (/FBT|ТЕРАП|ЛЕЧЕН/.test(up)) return map.FBT;
    if (/ULTRA|УЛЬТРА|UAI/.test(up)) return map.UAI;
    if (/ALL\s*INCLUSIVE|ВС[ЕЁ]\s*ВКЛ|AI/.test(up)) return map.AI;
    if (/FULL\s*BOARD|ПОЛН|FB/.test(up)) return map.FB;
    if (/HALF\s*BOARD|ПОЛУ|HB/.test(up)) return map.HB;
    if (/BREAKFAST|ЗАВТРАК|BB/.test(up)) return map.BB;
    return raw;
  };
  ...
  const sellingKb = (extraRows = []) => {
    ...
    if (isOwnerSide && (r === "provider" || r === "admin" || options?.forceShowProviderContacts === true)) {
      ...
      // Только админ может подготовить публичную карточку для канала/группы.
      // Поставщик эту кнопку не видит, чтобы не публиковать карточки от имени Travella.
      if (r === "admin" || options?.canPublishPublicSafe === true) {
  ...
  const getStayLines = () => {
    ...
    if (role === "provider" || role === "admin" || options?.forceShowProviderContacts === true) {
      ...
      if (role === "admin" || options?.canPublishPublicSafe === true) {
  ...
  if ((role !== "provider" || options?.forceRefused === true) && String(category) === "refused_tour") {
    ...
    if (accommodation) mainBits.push(accommodationLabel(accommodation));
    ...
    const included = [];
    const explicitNoFlight =
      hasNegativeFlag(d.flightIncluded, d.flightsIncluded, d.includesFlight, d.hasFlight, d.flight) ||
      /без\s+(?:авиа|перел[её]т)|no\s*flight|flight\s*not\s*included/i.test(String(d.flight || d.flightIncludedNote || d.flightNote || ""));

    // refused_tour creation/edit wizard asks flight fields, so the card must not
    // depend only on optional legacy flags like flightIncluded/includesFlight.
    ...
    if (hasPositiveFlag(d.arrivalFastTrack, d.fastTrack, d.fastTrackIncluded, d.arrivalFastTrackIncluded)) included.push("fast track");
    if (hasPositiveFlag(d.earlyCheckIn, d.earlyCheckInIncluded)) included.push("раннее заселение");
    if (hasPositiveFlag(d.lateCheckOut, d.lateCheckOutIncluded)) included.push("поздний выезд");

    if (included.length) {
      pushDivider(parts);
      parts.push("✅ <b>Включено:</b>");
      for (const item of included) parts.push(`• ${escapeHtml(item)}`);
    }
  ...
  if (accommodation) parts.push(`🛏 ${escapeHtml(accommodationLabel(accommodation))}`);
