...
const TG_CALLBACK_SECRET = (process.env.TG_CALLBACK_SECRET || "").trim();
...
function _cbSignUnlock({ action, chatId, serviceId, ts }) {
  const base = `${action}|${chatId}|${serviceId}|${ts}`;
  const secret = TG_CALLBACK_SECRET || "dev-secret";
  return crypto.createHmac("sha256", secret).update(base).digest("hex").slice(0, 12);
}

/**
 * Возвращает callback_data для unlock:
 * u:<serviceId>:<chatId>:<ts>:<sig>
 */
function buildUnlockCbData(chatId, serviceId) {
  const action = "u";
  const ts = _cbNowSec();
  const sig = _cbSignUnlock({
    action,
    chatId: Number(chatId),
    serviceId: Number(serviceId),
    ts,
  });
  return `${action}:${Number(serviceId)}:${Number(chatId)}:${ts}:${sig}`;
}

/**
 * Проверка подписи (используется в bot.js)
 */
...
module.exports = { buildServiceMessage, buildUnlockCbData, verifyUnlockCbData };
