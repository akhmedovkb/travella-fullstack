...
async function finishCreateServiceFromWizard(ctx) {
  try {
   ...
   if (grossNum === null) {
     await ctx.reply(
       "Не понял цену брутто. Пожалуйста, введите число, например 1250 или 1250 USD."
     );
  ...
  try {
    ...
    await ctx.reply(
      "✅ Готово!\n\n" +
        ...
        "Теперь бот сможет отправлять уведомления и показывать ваши разделы.",
      { parse_mode: "Markdown" }
    );
    ...
    await ctx.reply(
      "📝 Заявка принята!\n\n" +
        ...
        "Менеджер свяжется с вами после модерации.\n\n" +
        `🌐 Сайт: ${SITE_URL}`,
      { parse_mode: "Markdown" }
    );
...
bot.start(async (ctx) => {
  ...
  try {
    ...
    await ctx.reply(
      "👋 Добро пожаловать в Travella!\n\n" +
        "Чтобы бот показывал ваши бронирования/заявки, привяжем аккаунт по номеру телефона."
    );
    await askRole(ctx);
  } catch (e) {
...
bot.action(/^role:(client|provider)$/, async (ctx) => {
  try {
    ...
    await ctx.reply(
      role === "client"
        ? "👤 Роль: *Клиент*\n\n" +
            "📲 Отправьте номер телефона, который вы указали на сайте *travella.uz*.\n\n" +
            "Можно текстом: <code>+998901234567</code>\n" +
            "или нажмите кнопку ниже."
        : "🏢 Роль: *Поставщик*\n\n" +
            "📲 Отправьте номер телефона, который вы указали при регистрации на *travella.uz*.\n\n" +
