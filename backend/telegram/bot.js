...
async function finishEditWizard(ctx) {
  ...
  try {
    ...
    // ✅ обязательные поля перед сохранением (редактирование)
    ...
    if (!normReq(draft.country)) {
      const step = isHotelFlow ? "svc_edit_hotel_country" : "svc_edit_tour_country";
    ...
    if (isHotelFlow) {
      ...
      if (!normReq(draft.fromCity) || !normReq(draft.toCity)) {
        const step = !normReq(draft.fromCity) ? "svc_edit_tour_from" : "svc_edit_tour_to";
    ...
    // ✅ единственная проверка gross >= net (редактирование)
    if (draft.price != null && draft.grossPrice != null) {
      ...
      if (!ok) return;
      // editWizNavKeyboard() уже покажет кнопки назад/отмена на следующем запросе
      ctx.session.editWiz = ctx.session.editWiz || {};
...
// gross >= net (для create/edit). Если gross пустой/пропущен — ок.
async function validateGrossNotLessThanNet(ctx, netRaw, grossRaw, backToState) {
  const net = normalizePrice(netRaw);
  const grossText = String(grossRaw ?? "").trim();

  if (!grossText) return true;

  const gross = normalizePrice(grossText);
  if (gross === null) return true; // формат gross отдельно валидируется в месте ввода/сохранения

  if (net !== null && gross < net) {
    const isEdit =
      String(backToState || "").startsWith("svc_edit_") ||
      String(ctx.session?.editWiz?.step || "").startsWith("svc_edit_") ||
      String(ctx.session?.state || "").startsWith("svc_edit_");
    ...
    if (isEdit) {
      await safeReply(ctx, msg, { parse_mode: "Markdown", ...editWizNavKeyboard() });
      if (backToState) {
    ...
    await ctx.reply(msg, { parse_mode: "Markdown", ...wizNavKeyboard() });
...
async function finishCreateServiceFromWizard(ctx) {
  try {
    ...
    // ✅ обязательные поля перед сохранением
    if (!normReq(draft.title)) {
    ...
    if (!normReq(draft.country)) {
      ...
      ctx.session.state = category === "refused_hotel" ? "svc_hotel_country" : "svc_create_tour_country";
    ...
    if (category === "refused_hotel") {
      ...
      if (!normReq(draft.fromCity) || !normReq(draft.toCity)) {
        await ctx.reply("⚠️ Поля *Город вылета* и *Город прибытия* обязательны.", {
          parse_mode: "Markdown",
          ...wizNavKeyboard(),
        });
        ctx.session.state = !normReq(draft.fromCity) ? "svc_create_tour_from" : "svc_create_tour_to";
        await promptWizardState(ctx, ctx.session.state);
        return;
      }
    }
    ...
    // ✅ если брутто введено — проверяем gross >= net сразу (создание)
    if (draft.grossPrice != null) {
      const ok = await validateGrossNotLessThanNet(ctx, draft.price, draft.grossPrice, "svc_create_grossPrice");
      if (!ok) return;
    }
...
draft.grossPriceNum = grossNum;
    ...
    // ✅ финальная защита: gross >= net (создание)
    if (grossNumFinal != null && grossNumFinal < priceNum) {
      await ctx.reply(
        `⚠️ Цена *БРУТТО* не может быть меньше *НЕТТО*.\nНЕТТО: *${priceNum}*\nБРУТТО: *${grossNumFinal}*\n\nВведите корректную цену БРУТТО или напишите *пропустить*.`,
        { parse_mode: "Markdown", ...wizNavKeyboard() }
      );
      ctx.session.state = "svc_create_grossPrice";
      return;
    ...
    if (state && (state.startsWith("svc_create_") || state.startsWith("svc_hotel_"))) {
      ...
      switch (state) {
        ...
        case "svc_create_grossPrice": {
          ...
          // ✅ если брутто введено — проверяем что не меньше нетто сразу
          if (draft.grossPrice != null) {
            const ok = await validateGrossNotLessThanNet(ctx, draft.price, draft.grossPrice, "svc_create_grossPrice");
            if (!ok) return;
          }
