...
bot.hears(/👤 Профиль/i, async (ctx) => {
  ...
  if (!role) {
    // пробуем подтянуть поставщика из БД
  ...
  if (role === "provider") {
    await ctx.reply(
      "Ваш профиль поставщика можно дополнить и изменить в личном кабинете Travella во вкладке «Профиль».\n\n" +
...
bot.hears(/🧳 Мои услуги/i, async (ctx) => {
  ...
  // если роль не записана в сессии (бот перезапускался и т.п.) —
  // пробуем определить, является ли пользователь поставщиком
  if (!role) {
    role = await ensureProviderRole(ctx);
  }
