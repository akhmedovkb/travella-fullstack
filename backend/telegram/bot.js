...
function isTelegramAdminUserId(userId) {
  ...
  if (MANAGER_CHAT_ID) ids.add(String(MANAGER_CHAT_ID));
  return ids.has(id);
}
...
async function finishEditWizard(ctx) {
  ...
  try {
    ...
    const details = {
      ...
      urgency: getServiceUrgency(expirationValue).code,
      urgencyLabel: getServiceUrgency(expirationValue).label,
      urgencyBadge: getServiceUrgency(expirationValue).badge,
      urgencyPriority: getServiceUrgency(expirationValue).priority,
...
async function resolveRoleByUserId(userId, ctx) {
  if (isTelegramAdminUserId(userId)) {
    if (ctx && ctx.session) {
...
async function replyProviderDraftResumePrompt(ctx, row) {
  ...
  const stepLabels = {
    ...
    svc_create_flight_airline: "Авиакомпания",
...
function legacyUrgencyLabel(value) {
...
async function finishCreateServiceFromWizard(ctx) {
  try {
    ...
    if (details && typeof details === "object") {
      const computedUrgency = getServiceUrgency(draft.expiration || details.expiration || details.expiration_at);
      details.urgency = computedUrgency.code;
      details.urgencyLabel = computedUrgency.label;
      details.urgencyBadge = computedUrgency.badge;
      details.urgencyPriority = computedUrgency.priority;
    }
...
bot.start(async (ctx) => {
  ...
  try {
    ...
    if (role) {
      ...
      // ✅ Deep-link: unlock_<serviceId> => сразу открыть/купить контакты без повторной карточки
      const mUnlock = startPayloadRaw.match(/^unlock_(\d+)$/i);
      if (mUnlock) {
        const serviceId = Number(mUnlock[1]);
        if (!Number.isFinite(serviceId) || serviceId <= 0) {
          await ctx.reply("⚠️ Некорректная ссылка открытия контактов.");
          return;
        }
...
async function resolveContactButtonAudience(ctx) {
  ...
  if (String(ctx?.session?.role || "").toLowerCase() === "provider") {
    return { role: "provider", privileged: true, reason: "session_provider" };
  }

  const providerId = await resolveProviderIdByTelegramChatId(fromId).catch(() => null);
  if (providerId) return { role: "provider", privileged: true, providerId, reason: "provider_chat" };

  return { role: "client", privileged: false };
}

function normalizeTelegramUsernameForLink(value) {
  ...
  u = u.split(/[/?#]/)[0].trim();
  return /^[A-Za-z0-9_]{3,64}$/.test(u) ? u : "";
}

function pickServiceProviderContactFields(svc = {}) {
  const details = parseDetailsAny(svc.details);
  const provider = svc.provider && typeof svc.provider === "object" ? svc.provider : {};
  const name =
    svc.provider_name ||
    svc.providerName ||
    svc.provider_title ||
  ...
  const telegram =
    ...
    svc.telegram ||
    svc.telegram_username ||
    provider.telegram ||
    provider.telegram_username ||
    provider.username ||
    details.providerTelegram ||
    details.telegram ||
    "";
  ...
  const phone =
    svc.provider_phone ||
    svc.providerPhone ||
    svc.provider_contact ||
    svc.providerContact ||
    provider.phone ||
    provider.contact ||
    provider.telegram_phone ||
  ...
  return {
    name: String(name || "Поставщик").trim(),
...
async function sendPrivilegedProviderContacts(ctx, serviceId, audience = {}) {
  ...
  const { data } = await axios.get(`/api/telegram/service/${sid}`, {
    params: { role: audience?.role || "provider", chatId: targetChatId },
  });
  ...
  const svc = data.service;
  const contacts = pickServiceProviderContactFields(svc);
  const tgUsername = normalizeTelegramUsernameForLink(contacts.telegram);
  const serviceUrl = buildServiceUrl(sid);
  const title =
    getCanonicalServiceDisplayTitle({ ...svc, category: svc.category || svc.type }, { allowFallback: true, maxLength: 90 }) ||
    svc.title ||
    `Услуга #${sid}`;

  const lines = [
    `📞 <b>Контакты поставщика</b>`,
    ``,
    `📌 <b>${escapeHtml(String(title))}</b>`,
    `🤝 <b>${escapeHtml(contacts.name || "Поставщик")}</b>`,
  ];

  if (tgUsername) {
    lines.push(`📲 <b>Telegram:</b> <a href="https://t.me/${encodeURIComponent(tgUsername)}">@${escapeHtml(tgUsername)}</a>`);
  } else if (contacts.telegram) {
    lines.push(`📲 <b>Telegram:</b> ${escapeHtml(contacts.telegram)}`);
  }

  if (contacts.phone) lines.push(`☎️ <b>Телефон:</b> <code>${escapeHtml(contacts.phone)}</code>`);
  if (contacts.whatsapp && contacts.whatsapp !== contacts.phone) lines.push(`🟢 <b>WhatsApp:</b> <code>${escapeHtml(contacts.whatsapp)}</code>`);

  if (!contacts.telegram && !contacts.phone && !contacts.whatsapp) {
    lines.push(``, `⚠️ В карточке не указаны отдельные контактные поля. Проверьте профиль поставщика.`);
  }

  const buttons = [];
  if (tgUsername) buttons.push([{ text: "💬 Написать в Telegram", url: `https://t.me/${tgUsername}` }]);
  buttons.push([{ text: "🌐 Подробнее на сайте", url: serviceUrl }]);

  try { await ctx.answerCbQuery("✅ Контакты отправлены в личку", { show_alert: false }); } catch {}

  await bot.telegram.sendMessage(targetChatId, lines.join("\n"), {
    parse_mode: "HTML",
    disable_web_page_preview: true,
...
async function sendPublicSafeServiceCardToChannel(ctx, serviceId) {
  const sid = Number(serviceId || 0);
  if (!Number.isFinite(sid) || sid <= 0) {
    try { await ctx.answerCbQuery("⚠️ Некорректная услуга", { show_alert: true }); } catch {}
    return false;
  }

  if (!isTelegramAdminUserId(ctx?.from?.id)) {
    try { await ctx.answerCbQuery("⛔ Только админ может публиковать в канал", { show_alert: true }); } catch {}
    return false;
  }

  if (!PUBLIC_CHANNEL_CHAT_ID) {
    try {
      await ctx.answerCbQuery("⚠️ Не задан TELEGRAM_PUBLIC_CHANNEL_ID", { show_alert: true });
    } catch {}
    await safeReply(
      ctx,
      "⚠️ Канал публикации не настроен. Добавьте в Railway переменную TELEGRAM_PUBLIC_CHANNEL_ID (например @channelname или -100...)."
    );
    return false;
  }

  const { data } = await axios.get(`/api/telegram/service/${sid}`, {
    params: { role: "client", chatId: ctx?.from?.id || undefined },
  });

  if (!data?.success || !data?.service) {
    try { await ctx.answerCbQuery("❗️Услуга не найдена", { show_alert: true }); } catch {}
    return false;
  }
  ...
  // Защита от старых версий шаблона: если кнопки связаться нет — добавим её принудительно.
  const hasContactButton = rows.some((row) =>
    (row || []).some((btn) => String(btn?.text || "").includes("Связаться"))
  );
  ...
  try {
    if (photoUrl) {
      const photo = String(photoUrl).startsWith("tgfile:")
        ? String(photoUrl).replace(/^tgfile:/, "").trim()
        : photoUrl;
      await bot.telegram.sendPhoto(PUBLIC_CHANNEL_CHAT_ID, photo, {
        caption: text,
        parse_mode: "HTML",
        reply_markup: replyMarkup,
      });
    } else {
      await bot.telegram.sendMessage(PUBLIC_CHANNEL_CHAT_ID, text, {
        parse_mode: "HTML",
        disable_web_page_preview: true,
        reply_markup: replyMarkup,
      });
    ...
    try { await ctx.answerCbQuery("✅ Опубликовано в канал", { show_alert: false }); } catch {}
    await safeReply(ctx, "✅ Public-safe карточка опубликована в канал/группу. Кнопки сохранены, контакты скрыты.");
    return true;
  } catch (e) {
...
bot.action(/^publish_public:(\d+)$/, async (ctx) => {
  try {
    await sendPublicSafeServiceCardToChannel(ctx, Number(ctx.match?.[1]));
  } catch (e) {
    console.error("[tg-bot] publish_public action error:", e?.message || e);
    try { await ctx.answerCbQuery("⚠️ Ошибка публикации", { show_alert: true }); } catch {}
  }
});
...
bot.action(/^contacts:(\d+)$/, async (ctx) => {
  try {
    ...
    if (audience.privileged) {
      await sendPrivilegedProviderContacts(ctx, serviceId, audience);
      return;
    }
...
async function replyProviderSupportPrompt(ctx, serviceId = null) {
  try {
    ...
    if (getProviderSupportPaymentMode(settings) === PROVIDER_SUPPORT_PAYMENT_MODE_PAYME_CLICK) {
      await replyProviderSupportPaymeClickPrompt(ctx, settings, serviceId);
      return;
    }
...
function getProviderSupportPaymentMode(settings = null) {
  return String(settings?.payment_mode || PROVIDER_SUPPORT_PAYMENT_MODE_CARD).trim() === PROVIDER_SUPPORT_PAYMENT_MODE_PAYME_CLICK
    ? PROVIDER_SUPPORT_PAYMENT_MODE_PAYME_CLICK
    : PROVIDER_SUPPORT_PAYMENT_MODE_CARD;
}

async function getProviderSupportPaymentModeFromDb() {
  try {
    if (!pool) return PROVIDER_SUPPORT_PAYMENT_MODE_CARD;
    const settings = await getProviderSupportSettings(pool);
    return getProviderSupportPaymentMode(settings);
  } catch {
    return PROVIDER_SUPPORT_PAYMENT_MODE_CARD;
  }
}
...
function isClickClientNotFoundError(err) {
  const msg = String(err?.message || err?.click?.error_note || "").toLowerCase();
  const code = Number(err?.click?.error_code ?? err?.click?.error ?? 0);
  return (
    code === -15 ||
    msg.includes("не является пользователем click") ||
    msg.includes("not click") ||
    msg.includes("client") && msg.includes("click") ||
    msg.includes("пользовател") && msg.includes("click")
  );
}
...
async function askClickUnlockPhone(ctx, { clientId, serviceId, amountSum, badPhone = "", reason = "no_phone" } = {}) {
  const cid = Number(clientId || 0);
  const sid = Number(serviceId || 0);
  const amount = Math.trunc(Number(amountSum || 0));

  if (!ctx.session) ctx.session = {};
  ctx.session.pendingClickUnlock = {
    clientId: cid,
    serviceId: sid,
    amountSum: amount,
    badPhone: badPhone ? normalizeClickPhone(badPhone) : "",
    createdAt: Date.now(),
  };
...
async function handleClickUnlockPhoneInput(ctx, rawPhone) {
  ...
  if (!/^998\d{9}$/.test(phone)) {
    await safeReply(
      ctx,
      "⚠️ Номер Click должен быть в формате 998901234567. Отправьте номер ещё раз.",
      { reply_markup: buildClickPhoneKeyboard() }
    );
    return true;
  }
  ...
  const sid = Number(pending.serviceId || 0);
  const amount = Math.trunc(Number(pending.amountSum || 0));

  if (!Number.isFinite(cid) || cid <= 0 || !Number.isFinite(sid) || sid <= 0 || !amount) {
    ctx.session.pendingClickUnlock = null;
    ctx.session.state = null;
    await safeReply(ctx, "⚠️ Данные оплаты устарели. Откройте карточку услуги заново.", {
      reply_markup: getClientHomeKeyboard(),
    });
    return true;
  }
  ...
  try {
    const result = await sendClickUnlockContactInvoice(ctx, {
      clientId: cid,
      serviceId: sid,
      amountSum: amount,
      phoneNumber: phone,
      fromPhonePrompt: true,
    });
    ...
    console.error("[tg-bot] click phone invoice error:", e?.message || e);
    if (isClickClientNotFoundError(e)) {
      await askClickUnlockPhone(ctx, {
        clientId: cid,
        serviceId: sid,
        amountSum: amount,
        badPhone: phone,
        reason: "not_click_user",
      });
      return true;
    }
    await safeReply(ctx, "⚠️ Не удалось создать Click-счёт. Проверьте номер Click или попробуйте позже.", {
      reply_markup: getClientHomeKeyboard(),
    });
  }
  return true;
...
async function sendClickUnlockContactInvoice(ctx, { clientId, serviceId, amountSum, phoneNumber = "", fromPhonePrompt = false }) {
  const cid = Number(clientId || 0);
  const sid = Number(serviceId || 0);
  const amount = Math.trunc(Number(amountSum || 0));
  ...
  const phone = normalizeClickPhone(phoneNumber) || await getClientClickPhone(cid);
  if (!phone) {
    return askClickUnlockPhone(ctx, { clientId: cid, serviceId: sid, amountSum: amount, reason: "no_phone" });
  }
  ...
  try {
    ...
    if (isClickClientNotFoundError(e)) {
      return askClickUnlockPhone(ctx, {
        clientId: cid,
        serviceId: sid,
        amountSum: amount,
        badPhone: phone,
...
bot.action(/^support_project:pay_payme:(\d+):(\d+)$/, async (ctx) => {
  try {
    await safeCb(ctx);
    if (await getProviderSupportPaymentModeFromDb() !== PROVIDER_SUPPORT_PAYMENT_MODE_PAYME_CLICK) {
      await replyProviderSupportCardDetails(ctx);
      return;
    }
    ...
    await safeReply(ctx, "⚠️ Не удалось обработать поддержку проекта. Попробуйте позже.");
  }
});
...
bot.action(/^unlock:click:(\d+)$/, async (ctx) => {
  try {
    await safeCb(ctx);
    ...
    if (!isClickConfigured()) {
      await safeReply(ctx, "⚠️ Click сейчас не настроен на сервере.");
      return;
    }
    ...
    console.error("[tg-bot] unlock:click error:", e?.message || e);
    try { await safeReply(ctx, "⚠️ Не удалось создать Click-счёт. Попробуйте позже."); } catch {}
  }
});
  ...
  try {
    ...
    case "svc_create_title": {
        ...
        case "svc_create_grossPrice": {
          ...
          // Срочность не спрашиваем вручную: она вычисляется из срока актуальности.
          ctx.session.state = "svc_create_expiration";
          await promptWizardState(ctx, "svc_create_expiration");
          return;
        }
        case "svc_create_urgency": {
          // Legacy fallback для старых сессий после деплоя: ручной шаг больше не используется.
          ctx.session.state = "svc_create_expiration";
          await promptWizardState(ctx, "svc_create_expiration");
          return;
        }
